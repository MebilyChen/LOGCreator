...
.draw优化
优化了骰子合并 增加了多轮掷骰
活字格式优化：更新文字效果；修改了名牌里如果有【】活字输出会乱码的bug；伤害掷骰格式由[技能名]伤害改成了[技能名伤害]
修改了小地图修改角色标签的行为（修改标签不记录，因此生成新标签时使用的是旧数据，用于参考 → 修改标签后会记录，因此生成新标签时使用的是新数据，无法参考但能保证更新）
多轮掷骰优化
解决了无法增加角色栏的BUG
output log可自定义命名
回车环境状态栏可发送至log
优化了小地图导入图片，避免卡死；小地图隐藏文字标签，大型背景图置于底层
一些小修补，增加了自动保存（10分钟间隔）
多项修补+功能添加
优化了巴别塔bug
调查模块双击条目可查看全部信息
优化适配回声
保存时文件名会显示上一个起的名字（同次软件开启下）
调整了部分时间模块
增加骰子性格（妙语），增加jrrp
增加右键菜单、技能列表
添加NPC战斗额外窗口,优化，增加随机怪物和数量
技能、状态、NPC模板可程序内添加（不用再自己编辑JSON文件了，但巴别塔、PL预设物品备注等还需要编辑文件）
NPC随机生成
增加情报扩散功能
优化canvas，能够同步时间、新增的PC/NPC与PC状态
各种更新
1、小窗功能：KP栏触发，可指定当前一个role发送KP窗口信息，程序会处理成乱码再发送至log，在输出log时恢复，可通过在说话人栏位输入乱码发送解密查看
self.whisper_data[说话人] = {“接收人1”：["乱码1"："说话内容1"，"乱码2"："说话内容2"]，“接收人2”：["乱码3"："说话内容3"，"乱码4"："说话内容4"]}}
2、SAN值变动同步至角色卡而不只是栏位卡
3. 优化了treeview超出屏幕的现象
4. 程序能够续接音效、BGM、HO和FX 
5. 程序会显示当前正在播放的BGM名称
6. BGM播放列表生成+标记
7. 各种杂七杂八更新
...
6. 技能可以选多个敌人，可以存在多个算式，增加了法术的成功率设置
7. 战斗入场出场台词
8. 先攻顺序
